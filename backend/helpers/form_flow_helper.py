...
FILL_ROLES = [User.ROLE_CLIENT, User.ROLE_PARALEGAL, User.ROLE_ATTORNEY]
...
def assert_user_can_fill(user: dict) -> None:
    if user.get("role") not in FILL_ROLES:
        raise HTTPException(
            ...
            detail="Only client, paralegal, or attorney accounts may open and submit these forms.",
        )
...
async def admin_upload_template(
    *,
    ...
    msg = "Template saved and fields detected from placeholders."
    if not schema:
        msg = "Template saved but no placeholders matched. Use {{field_name}} or [[FIELD_NAME]] in the document."
